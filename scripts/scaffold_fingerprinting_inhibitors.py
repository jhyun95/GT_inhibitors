...
if __name__ == '__main__':
    ...
    inhibitor_generate = subprocess.run(['sng', 'generate', '-p', '100', '-o', '../data/temp.tmp', '../data/temp.smi'], check=True, text=True)
    inhibitor_aggregate = subprocess.run(['sng', 'aggregate', '-m', '../data/temp_m.tmp', '-o', '../data/temp.network', '../data/temp.tmp'], check=True, text=True)
    ...
    pd.DataFrame(data=molecule_scaffold_d).to_csv('../data/scaffold_fingerprints_inhibitors.csv')